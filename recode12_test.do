...
drop *_01
replace male = .a in 4
recode12 male, suffix(_x)
assert male_x == .a in 4
assert male_x == 1 if male == 1
assert male_x == 0 if male == 2

capture noisily recode12 male, suffix(_x)
assert _rc == 110

recode12 male, replace
assert male == .a in 4
...
capture noisily recode12 x, suffix(_z) replace
assert _rc == 198
assert x[1] == 1 & x[2] == 2
