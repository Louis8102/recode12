...
* 2. Eligible string variables only.  The first distinct nonblank category
...
use recode12_example_data.dta, clear
recode12 owns_car home_owner_text completed_training, yesvalue(2)
list id owns_car owns_car_01 home_owner_text home_owner_text_01 ///
    completed_training completed_training_01 in 1/8, separator(0)

* 4. With no varlist, all eligible numeric and string variables are processed;
* variables with one, three, or no nonblank string categories are skipped.
use recode12_example_data.dta, clear
recode12, yesvalue(2)
return list
